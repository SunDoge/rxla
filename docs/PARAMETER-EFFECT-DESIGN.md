...
```rust
use rxla::{Tensor, nn::{Cx, Layer, Linear, Model, ModelInput, Result}};

fn apply(cx: &mut Cx, x: Tensor) -> Result<Tensor> {
    let hidden = Linear::new(8).named("hidden");
    let head = Linear::new(3).named("head");
    let x = hidden.apply(cx, &x)?.relu()?;
    head.apply(cx, &x)
}
...
consuming setters generated by `derive_setters`; `.named(name)` binds a stable
relative identity without borrowing `Cx`, and `apply(cx, input)` enters that
parameter scope and records effects. There is no
separate builder type or terminal `build()` call:

```rust
# use rxla::{Tensor, nn::{Conv2d, Cx, Layer, Result}};
# fn block(cx: &mut Cx, image: Tensor) -> Result<Tensor> {
let stem = Conv2d::new(64, [3, 3]).bias(false).named("stem");
let hidden = stem.apply(cx, &image)?;
# Ok(hidden)
# }
```
