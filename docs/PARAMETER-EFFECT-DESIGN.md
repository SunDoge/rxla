...
fn apply(cx: &mut Cx, x: Tensor) -> Result<Tensor> {
    let x = Linear::new(8).apply(cx, "hidden", &x)?.relu()?;
    Linear::new(3).apply(cx, "head", &x)
}
...
consuming setters generated by `derive_setters`; `apply(cx, name, input)` is the
only step that enters a parameter scope and records effects. There is no
