# Rust tensor / Pliron / StableHLO / PJRT

This is the standalone `rxla` workspace. Commands below are run from this
directory.

Start with [current scope and remaining work](docs/SCOPE.md) for a compact overview;
...
[design constraints and staged acceptance gates](docs/EXECUTION-PLAN-DESIGN.md)
...
The [MLX reference review](docs/MLX-LAYOUT-REVIEW.md) records adopted layout/view,
...
and optional managed F32/I32/BF16 input storage. See [ownership and migration notes](docs/TENSOR-STORAGE-DESIGN.md)
...
sequence measurements are recorded in [LOGSCAN-BENCHMARK.md](docs/LOGSCAN-BENCHMARK.md):
...
The opt-in [placement benchmark](docs/HETEROGENEOUS-BENCHMARK.md) compares whole CPU,
...
With the CUDA dependencies configured as in [CUDA validation](docs/CUDA-validation.md):
...
see [pinned dependencies, reproduction commands and limits](docs/CUDA-validation.md).
...
and a whole graph, see [pipeline measurements](docs/PIPELINE-BENCHMARK.md). The measured
...
- `rxla-weights`: optional seek-based safetensors reader and F32 weight uploads;
  see [crates/rxla-weights/README.md](crates/rxla-weights/README.md). Not a dependency of `rxla-core`.
...
test. State-management API constraints are documented in [STATE-DESIGN.md](docs/STATE-DESIGN.md);
