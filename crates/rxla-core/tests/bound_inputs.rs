...
fn real_many_interleaved_fixed_inputs() {
    ...
    for index in 0..64 {
        ...
        bindings.push((index, Rc::new(client.buffer(&[], &[index as f32]).unwrap())));
    }
...
fn real_fixed_inputs_are_shared_validated_and_rebindable() {
    ...
    let mut second = program
        ...
        .unwrap();
    let weight = Rc::new(client.buffer(&[2], &[2., 3.]).unwrap());
    let one = Rc::new(client.buffer(&[], &[1.]).unwrap());
    let position = Rc::new(client.buffer(&[], &[1]).unwrap());
    assert_eq!(first.input_count(), 3);
    first
        .bind_inputs(vec![(2, position.clone()), (1, weight.clone())])
        .unwrap();
    second.bind_inputs(vec![(1, weight.clone())]).unwrap();
    assert_eq!(Rc::strong_count(&weight), 3);
    ...
    let invalid = vec![
        ...
        vec![(1, one.clone())],
        vec![(2, one.clone())],
        vec![(1, Rc::new(foreign.buffer(&[2], &[4., 5.]).unwrap()))],
    ];
