...
use rxla_nn::{
    AppliedModel, BatchNorm, Conv2d, Cx, ExecutionMode, Layer, Linear, Model, ModelInput,
    Result as NnResult,
};
...
fn basic_block(
    ...
    let conv1 = Conv2d::new(channels, [3, 3])
        ...
        .bias(false)
        .named("conv1");
    let bn1 = BatchNorm::new().named("bn1");
    let conv2 = Conv2d::new(channels, [3, 3])
        .options(Conv2dOptions {
            padding: [[1, 1], [1, 1]],
            ..Default::default()
        })
        .bias(false)
        .named("conv2");
    let bn2 = BatchNorm::new().named("bn2");

    let hidden = conv1.apply(&mut block, input)?;
    let hidden = bn1.apply(&mut block, &hidden)?.relu()?;
    let hidden = conv2.apply(&mut block, &hidden)?;
    let hidden = bn2.apply(&mut block, &hidden)?;
    let residual = if input.shape()[3] == channels && stride == 1 {
        input.clone()
    } else {
        let shortcut_conv = Conv2d::new(channels, [1, 1])
            .options(Conv2dOptions {
                strides: [stride, stride],
                ..Default::default()
            })
            .bias(false)
            .named("shortcut_conv");
        let shortcut_bn = BatchNorm::new().named("shortcut_bn");
        let projected = shortcut_conv.apply(&mut block, input)?;
        shortcut_bn.apply(&mut block, &projected)?
    };
...
fn resnet18(cx: &mut Cx, images: Tensor, labels: Tensor) -> NnResult<(Tensor, Tensor, Tensor)> {
    ...
    let stem_conv = Conv2d::new(64, [7, 7])
        .options(Conv2dOptions {
            strides: [2, 2],
            padding: [[3, 3], [3, 3]],
            ..Default::default()
        ...
        .bias(false)
        .named("stem_conv");
    let stem_bn = BatchNorm::new().named("stem_bn");
    let head = Linear::new(CLASSES).named("head");

    let mut hidden = stem_conv.apply(cx, &images)?;
    hidden = stem_bn.apply(cx, &hidden)?.relu()?.avg_pool2d(
        Pool2dOptions {
            window: [3, 3],
            strides: [2, 2],
            padding: [[1, 1], [1, 1]],
    ...
    let logits = head.apply(cx, &features)?;
    let loss = logits
